...
export const useUploadStore = create((set, get) => ({
    ...
    pasteTransitionToSelected() {
        ...
        const updated = clips.map((c, idx) => selectedIds.includes(c.id) && idx !== lastIndex ? { ...c, transitionAfter: { ...clipboardTransition } } : c);
        const enforced = enforceConstraints(updated);
        set({ clips: enforced });
        void persist(enforced);
    },
    async hydrate() {
        const saved = (await idbGet(CLIPS_KEY));
...
function enforceConstraints(clips) {
